...
const part1 = () => {
    ...
    while (ptr<arr.length) {
        ...
        switch (digit) {
            case "1":
                arr = handleAddition(arr, ptr);
                ptr += 4;
            break;
            case "2":
                arr = handleMultiply(arr, ptr);
                ptr += 4;
            break;
            case "3":
            ...
            break;
            case "4":
                output = outputValueAtAddress(arr, parseInt(ptr)+1);
...
const handleInstruction = (arr, ptr) => {
    ...
    switch (opCode) {
        ...
        case 2:
            ...
            var saveToAddress = getValueInAddress(arr, ptr+3);
            console.log(`save to addr: ${newValue}`);
        ...
        case 4:
            val1 = getValueInAddress(arr, ptr+1);
            console.log(val1);
            return {
                arr: arr,
                newPtr: (ptr+2)
...
const getLastTwoDigit = (instruction) => {
    ...
    return parseInt(instruction.toString().substring(instruction.length-2));
}

const handleAddition = (arr, ptr) => {
    var val1 = getValueInAddress(arr, ptr+1);
    val1 = arr[val1];
    var val2 = getValueInAddress(arr, ptr+2);
    val2 = arr[val2];
    var saveToAddress = getValueInAddress(arr, parseInt(ptr)+3)
    var newValue = add(val1, val2);
    arr[saveToAddress] = newValue;
    console.log(`${saveToAddress}, ${val1}, ${val2}, ${ptr}`);
    return arr;
}

const handleMultiply = (arr, ptr) => {
    var val1 = getValueInAddress(arr, parseInt(ptr)+1);
    val1 = arr[val1];
    var val2 = getValueInAddress(arr, parseInt(ptr)+2);
    val2 = arr[val2];
    var saveToAddress = getValueInAddress(arr, parseInt(ptr)+3)
    var newValue = multiply(val1, val2);
    arr[saveToAddress] = newValue;
    return arr;
}

const handleInput = (arr, input, ptr) => {
    var saveToAddress = getValueInAddress(arr, parseInt(ptr)+1);
...
const outputValueAtAddress = (arr, outputAddress) => {
    return arr[outputAddress];
}

const getValueInAddress = (arr, index) => {
    return arr[parseInt(index)];
}
// var arr = data.split(",");
// var n = getValueInAddress(arr, 6);
// var n2 = getValueInAddress(arr, 225);
