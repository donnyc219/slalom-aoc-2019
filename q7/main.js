...
const runAmplifier = (inputs) => {
    ...
    while (ptr<arr.length) {
        ...
        arr = res.arr;
        ptr = res.newPtr;
        finalResult = (res.finalResult!=null)? res.finalResult: finalResult;
    }
    return finalResult;
...
const part1 = () => {
    ...
    for (var i in arr) {
        numStr = arr[i];
        output = getOutput(numStr);
        resArray.push(output);
    }
    return Math.max(...resArray);
}

const getOutput = (numStr) => {
    var n;
    var digit;
    var prevOutput = 0;
    for (var i in numStr) {
        prevOutput = runAmplifier([numStr[i], prevOutput]);
    }
    return prevOutput;
}


// var n = getOutput("01234");

// part1();
// var n = runAmplifier([1, 0]);
// n = runAmplifier([0, n]);
// n = runAmplifier([4, n]);
// n = runAmplifier([3, n]);
// n = runAmplifier([2, n]);
